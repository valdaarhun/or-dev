...
static struct kprobe kp = {
    .symbol_name = "_syscall_call",
};

static int my_pre_handler_hello(struct kprobe *p, struct pt_regs *regs) {
    printk(KERN_INFO "Inside pre-handler... Processing KProbe (do_trap)\n");
    return 0;
}

static void my_post_handler(struct kprobe *p, struct pt_regs *regs, unsigned long flags) {
    printk(KERN_INFO "Inside post-handler... Finishing KProbe processing (do_trap)\n");
}
